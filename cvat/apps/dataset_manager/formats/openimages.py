...
def _import(src_file, temp_dir, instance_data, load_data_callback=None, **kwargs):
        ...
        for item_id in item_ids:
            ...
            try:
                frame_id = match_dm_item(
                    DatasetItem(id=item_id), instance_data, root_hint
                )
    ...
    detect_dataset(
        temp_dir,
        format_name="open_images",
        importer=dm_env.importers.get("open_images"),
    )
    dataset = Dataset.import_from(
        temp_dir, "open_images", image_meta=image_meta, env=dm_env
    )
