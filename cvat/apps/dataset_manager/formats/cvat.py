    ...
    for frame_annotation in annotations.group_by_frame(include_empty=True):
        ...
        def dump_labeled_shapes(shapes, is_skeleton=False):
            for shape in shapes:
                ...
                if is_skeleton:
                    dump_data.update(
                        OrderedDict([("outside", str(int(shape.outside)))])
                    )

                if shape.type != "skeleton":
                    dump_data.update(
                        OrderedDict([("occluded", str(int(shape.occluded)))])
                    )
                    ...
                    dump_data.update(
                        OrderedDict(
                            [
                                ...
                                (
                                    "rx",
                                    "{:.2f}".format(shape.points[2] - shape.points[0]),
                                ),
                                (
                                    "ry",
                                    "{:.2f}".format(shape.points[1] - shape.points[3]),
                                ),
                            ]
                        )
                    )
                    ...
                    dump_data.update(
                        OrderedDict(
                            [
                                (
                                    "rle",
                                    f"{list(int (v) for v in shape.points[:-4])}"[1:-1],
                                ),
                                ("left", f"{int(shape.points[-4])}"),
                                ("top", f"{int(shape.points[-3])}"),
                                (
                                    "width",
                                    f"{int(shape.points[-2] - shape.points[-4]) + 1}",
                                ),
                                (
                                    "height",
                                    f"{int(shape.points[-1] - shape.points[-3]) + 1}",
                                ),
                            ]
                        )
                    )
                ...
                for attr in shape.attributes:
                    dumper.add_attribute(
                        OrderedDict([("name", attr.name), ("value", attr.value)])
                    )
    ...
    def dump_track(idx, track):
            ...
            dump_data.update(
                OrderedDict(
                    [
                        ...
                        (
                            "subset",
                            get_defaulted_subset(task.subset, annotations.subsets),
                        ),
                    ]
                )
            )
        ...
        for element_track in track.elements:
            for element_shape in element_track.shapes:
                ...
                element_shapes[element_shape.frame].append(
                    (element_shape, element_track.label)
                )
...
def load_anno(file_object, annotations):
    ...
    for ev, el in context:
        if ev == "start":
            ...
            if (
                el.tag == "attribute"
                and elem_attributes is not None
                and shape_element is not None
            ):
            ...
            if (
                ...
                if el.tag == "box":
                    ...
                    shape_element["points"].append(
                        "{:.2f}".format(float(el.attrib["cx"]) + float(el.attrib["rx"]))
                    )
                    shape_element["points"].append(
                        "{:.2f}".format(float(el.attrib["cy"]) - float(el.attrib["ry"]))
                    )
                elif el.tag == "cuboid":
                ...
                if el.tag == "box":
                    ...
                    shape["points"].append(
                        "{}".format(
                            int(el.attrib["left"]) + int(el.attrib["width"]) - 1
                        )
                    )
                    shape["points"].append(
                        "{}".format(
                            int(el.attrib["top"]) + int(el.attrib["height"]) - 1
                        )
                    )
                elif el.tag == "cuboid":
...
def dump_project_anno(
    dst_file: BufferedWriter, project_data: ProjectData, callback: Callable
):
    dumper = create_xml_dumper(dst_file)
    dumper.open_document()
...
def dump_media_files(
    instance_data: CommonData, img_dir: str, project_data: ProjectData = None
):
    ext = ""
    ...
    for frame_id, (frame_data, _) in zip(instance_data.rel_range, frames):
        if (
            ...
            and (instance_data.db_instance.id, frame_id) in project_data.deleted_frames
        ) or frame_id in instance_data.deleted_frames:
            continue
        frame_name = (
            instance_data.frame_info[frame_id]["path"]
            if project_data is None
            else project_data.frame_info[(instance_data.db_instance.id, frame_id)][
                "path"
            ]
        )
...
def _export_task_or_job(
    dst_file, temp_dir, instance_data, anno_callback, save_images=False
):
...
def _export_project(
    ...
    if save_images:
        for task_data in project_data.task_data:
            subset = get_defaulted_subset(
                task_data.db_instance.subset, project_data.subsets
            )
